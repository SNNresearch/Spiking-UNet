...
from . import help_functions as hp
...
def get_data_training(DRIVE_train_imgs_original,
                      ...
                      N_subimgs,
                      inside_FOV):
    train_imgs_original = hp.load_hdf5(DRIVE_train_imgs_original)
    train_masks = hp.load_hdf5(DRIVE_train_groudTruth) #masks always the same
    # hp.visualize(hp.group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train')#.show()  #check original imgs train


    train_imgs = pre.my_PreProc(train_imgs_original)
...
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ### test
    test_imgs_original = hp.load_hdf5(DRIVE_test_imgs_original)
    test_masks = hp.load_hdf5(DRIVE_test_groudTruth)

    test_imgs = pre.my_PreProc(test_imgs_original)
...
def recompone(data,N_h,N_w):
    ...
    patch_h = int(data.shape[1])
    patch_w = int(data.shape[2])
    N_pacth_per_img = N_w*N_h
